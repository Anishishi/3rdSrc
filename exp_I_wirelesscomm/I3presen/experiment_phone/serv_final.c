...
//rec -t raw -b 16 -c 1 -e s -r 44100 - | nc -l 50000 | play -t raw -b 16 -c 1 -e s -r 44100 -
...
void *exe_recv(void *tmp){
    ...
    int* s=(int *)tmp;
    while(1){
        ssize_t m=recv(*s, data, NMAX*sizeof(sample_t), 0);
        if(m==-1 || m==0) continue;
        write_n(1, m, data);//nバイトもらって一文字ずつputcharできない…
    }
}
...
void exe_send(int s){
    ...
    int p_num=fileno(p);
    while(1){
        ssize_t m=read_n(p_num, NMAX * sizeof(sample_t), buf);//m文字読み込む
        if(m==0 || m==-1) continue;
        for(int i=0; i<m; i++){
        ...
        int n1=send(s, buf, m, 0);//必要最低限だけ送る。
        if(n1==-1) continue;
    }
    fclose(p);
    //exit(1);
